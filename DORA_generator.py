...
def Duff_oscillator(t, q: np.ndarray,
                       c: float = 0.1,
                       k: float = 1.0,
                       f: float = 0,
                       omega: float = 1.0,
...
def SolveDuffing(q0, t_eval, c, k, f, omega, phi, beta):
    ...
    sol_forced = solve_ivp(Duff_oscillator, t_span=[t_eval[0], t_eval[-1]], y0=q0,\
...
def ForcingRespose(q0, t_eval, c, k, f_steps, omega, phi, beta, EvalTransients):
    ...
    sol = solve_ivp(Duff_oscillator, t_span=[t_eval[0], t_eval[-1]], y0=q0,\
...
def GenerateData(Time, Plot_TF):
    ...
    solA = solve_ivp(Duff_oscillator, t_span=[t_eval[0], t_eval[-1]], y0=q0,\
                          t_eval=t_eval, args=(c, k, fs[0], omega, phi, beta))
    
    solB = solve_ivp(Duff_oscillator, t_span=[t_eval[0], t_eval[-1]], y0=q0,\
